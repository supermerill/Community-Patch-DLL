...
-- Top Panel Tooltip Adjustment (make generic for more than just founder beliefs)
	...
	UPDATE Language_fr_FR
	SET Text = 'Une ville a une Puissance de combat àdistance égale à sa Force au début du combat, et a une portée de 1. Cette portée augmente au long du jeu en fonction des recherches technologiques (regardez les icônes "attaque à distance" dans l''arbre des technologies). Elle peut attaquer toute unitée ennemie présent à cette distance. Notez que la puissance de combat à distance de la ville ne décroît pas avec les dégats, elle reste à son mximum jusqu''à sa capture.'
