...
int CvUnit::GetMaxRangedCombatStrength(const CvUnit* pOtherUnit, const CvCity* pCity, bool bAttacking, bool bForRangedAttack) const
{
	...
	if(getKamikazePercent() != 0)
		...
		//handicap for combat
		CvPlayerAI *kDefenderPlayer = NULL;
		if(pOtherUnit != NULL){
			kDefenderPlayer = &GET_PLAYER(pOtherUnit->getOwner());
		}else if(pCity != NULL){
			kDefenderPlayer = &GET_PLAYER(pCity->getOwner());
		}
		if(kDefenderPlayer != NULL){
			...
			else if(kDefenderPlayer != NULL
				&& CvPreGame::slotStatus(kPlayer.GetID()) == SS_TAKEN
				&& CvPreGame::slotStatus(kDefenderPlayer->GetID()) == SS_TAKEN)
			{
				...
				int otherBonus = kDefenderPlayer->getHandicapInfo().getRangeAttackDmgMultHumanAgainstHuman();
